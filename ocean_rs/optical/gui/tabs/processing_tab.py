...
def create_processing_tab(gui, notebook):
    ...
    ttk.Label(perf_frame, text="GPT Memory (GB):").pack(side=tk.LEFT)
    ttk.Spinbox(
        perf_frame, from_=4, to=512, width=5,
        textvariable=gui.memory_limit_var,
        validate='key', validatecommand=vcmd
    ).pack(side=tk.LEFT, padx=(5, 20))

    ttk.Label(perf_frame, text="GPT Threads:").pack(side=tk.LEFT)
    ttk.Spinbox(
        perf_frame, from_=1, to=128, width=5,
