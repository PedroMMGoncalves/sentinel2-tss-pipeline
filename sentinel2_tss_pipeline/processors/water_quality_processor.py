...
def create_water_quality_processor(config: WaterQualityConfig = None) -> WaterQualityProcessor:
    """
    Factory function to create a water quality processor.

    Args:
        config: Water quality configuration
...
def process_water_quality_from_c2rcc(c2rcc_results: Dict,
                                     jiang_results: Dict,
                                     water_quality_config: WaterQualityConfig) -> Dict:
    """
    Process water quality products from C2RCC and Jiang TSS results.

    Args:
        c2rcc_results: Results from C2RCC processing
        jiang_results: Results from Jiang TSS processing
        water_quality_config: Configuration for water quality algorithms

    Returns:
        Dictionary with all water quality products
    """
    try:
        logger.info("Processing water quality products from C2RCC results")

        processor = create_water_quality_processor(water_quality_config)
        advanced_results = {}
        ...
        logger.info(f"Water quality processing completed: {len(advanced_results)} products generated")
        return advanced_results

    except Exception as e:
        logger.error(f"Error processing water quality products: {e}")
        return {}
